        ...
        $this->addSql("
                ...
                BEGIN
                    IF NEW.com_public = 1 THEN
                      ...
                      set @positiveRanking = IF(@total = 0, 0, IF(@total = 1, 1, IF(@positivePercent = 100, 5, 3)));
                      set @negativeRanking = IF(@negative = 0, 0, IF(@negativePercent >= 50, 5, IF(@negativePercent < 33, 1, 3)));

                      IF @exists = 0 THEN
                            INSERT INTO ownership_ranking_extra (accommodation,startDate,endDate,positiveComments, negativeComments,rankingPoints)
                            VALUES (@accommodation,@firstOfCurrentMonth,@lastOfCurrentMonth, @positiveRanking, @negativeRanking, @rankingPoints);
                        ELSE
