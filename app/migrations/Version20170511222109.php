 ...
 */
class Version20170511222109 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        $this->addSql("
        CREATE PROCEDURE calculateRankingVariables(IN monthValue INT, IN yearValue INT, calculateRanking TINYINT)
        BEGIN
          ...
          read_loop: LOOP
			...
			set @penalties = 0;
            set @awards = 0;
            set @totalFacturation = 0;
            set @currentMonthFacturation = 0;
            set @totalAvailableFacturation = 0;
            set @totalNonAvailableFacturation = 0;
            set @percent = 0;
			set @percent = 0;
			set @totalFacturation = 0;
			set @currentMonthFacturation = 0;
			...
			set @totalNonAvailableFacturation = 0;
			set @faRanking = 0;
			set @accommodationFailureType = 0;
			set @touristFailureType = 0;
			set @accommodationFailures = 0;
			set @touristFailures = 0;
			set @accommodationFailuresRanking = 0;
			set @touristFailuresRanking = 0;
			set @totalNewOffers = 0;
			set @payedNewOffers = 0;
			set @payedNewOffersPercent = 0;
			set @payedNewOffersRanking = 0;
			set @totalComments = 0;
			set @positive = 0;
			set @negative = 0;
			set @positivePercent = 0;
			set @negativePercent = 0;
			set @positiveRanking = 0;
			set @negativeRanking = 0;
			set @totalSolicitudesDisponibles = 0;
			set @reservations = 0;
			set @reservationsPercent = 0;
			set @reservationsRanking = 0;
			set @totalSolicitudes = 0;
			set @snd = 0;
			set @sdPercent = 0;
			set @sndPercent = 0;
			set @sdRanking = 0;
            ...
            set @penalties = (SELECT COUNT(*) FROM penalty p WHERE ((p.creationDate <= @lastOfCurrentMonth AND p.finalizationDate >= @lastOfCurrentMonth) OR (p.creationDate <= @firstOfCurrentMonth AND p.finalizationDate >= @firstOfCurrentMonth)) AND p.accommodation = accommodation);
            ...
            set @currentMonthFacturation = (select COALESCE(SUM(IF((owres.own_res_night_price is null or owres.own_res_night_price = 0) , COALESCE(owres.own_res_total_in_site, 0), owres.own_res_night_price * DATEDIFF(owres.own_res_reservation_to_date, owres.own_res_reservation_from_date))), 0) as total
                          ...
                          and gres.gen_res_own_id = accommodation
                          and gres.gen_res_status = 2
                          and owres.own_res_reservation_from_date >= @firstOfCurrentMonth and owres.own_res_reservation_from_date <= @lastOfCurrentMonth
                          and p.status in (1, 4));
            ...
            set @visits = COALESCE((SELECT d.visitsLastWeek FROM ownershipdata d WHERE d.accommodation = accommodation LIMIT 1), 0);
            ...
            IF @exists = 0 THEN
               ...
               currentMonthFacturation, totalFacturation, totalAvailableRooms, totalNonAvailableRooms, totalReservedRooms,
               visits, totalAvailableFacturation, totalNonAvailableFacturation)
               ...
               @currentMonthFacturation, @totalFacturation, @totalSolicitudesDisponibles, @snd, @reservations, @visits, @totalAvailableFacturation, @totalNonAvailableFacturation);
            ELSE
               ...
               SET rank.penalties = @penaltiesRanking,
                   ...
                   rank.totalFacturation = @totalFacturation,
                   rank.totalAvailableRooms = @totalSolicitudesDisponibles,
