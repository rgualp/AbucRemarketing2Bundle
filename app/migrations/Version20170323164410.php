 ...
 */
class Version20170323164410 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        $this->addSql("
                ...
                BEGIN
                    ...
                    IF NEW.own_res_status = 5  THEN
						set @accomm = (SELECT ownership.own_id FROM room INNER JOIN ownership ON (room.room_ownership = ownership.own_id) INNER JOIN ownershipreservation ON (room.room_id = ownershipreservation.own_res_selected_room_id) WHERE ownershipreservation.own_res_id = NEW.own_res_id LIMIT 1);
           				UPDATE ownership SET own_hot_date = NOW() WHERE ownership.own_id = @accomm;
		    		END IF;
