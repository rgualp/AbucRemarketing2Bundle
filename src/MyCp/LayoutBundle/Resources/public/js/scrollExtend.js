            ...
            if ( jqContainerElem.data('scrollExtend-disabled') != true &&
                ...
                localAjaxSettings = {
                    'success': function(data, textStatus) {
                        HoldOn.close();
                        Dashboard.setStart(Dashboard.getLimit()+1);
                        Dashboard.setLimit(Dashboard.getLimit()+5);
                        ajaxSettings.data.start=Dashboard.getStart();
                        ajaxSettings.data.limit=Dashboard.getLimit();
                        var target = ( settings.target ) ? settings.target : container;
