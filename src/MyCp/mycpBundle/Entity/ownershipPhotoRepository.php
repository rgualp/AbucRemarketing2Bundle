...
class ownershipPhotoRepository extends EntityRepository
{
    ...
    {
        $em = $this->getEntityManager();
        $newPrincipalPhoto = null;
        if ($accommodation != null) {
            $result = $this->checkAccommodationCoverPage($accommodation);
            if (count($result) > 0) {
                $newPrincipalPhoto = $result[0];
            } else {
    ...
    function checkAccommodationCoverPage($accommodation)
    {
        $query = $this->createQueryBuilder('ownershipPhoto');
        return $query->where('ownershipPhoto.own_pho_own = :accommodation')
            ->setParameter("accommodation", $accommodation->getOwnId())
            ->select('ownershipPhoto')
            ->innerJoin('ownershipPhoto.own_pho_photo', 'own_pho_photo')
            ->andWhere('own_pho_photo.front = true')
            ->getQuery()->getResult();
    ...
    public
    function createPhotoFromRequest(ownership $ownership, UploadedFile $file, $container, $post, $photoIndex = null)
    {
        $em = $this->getEntityManager();
