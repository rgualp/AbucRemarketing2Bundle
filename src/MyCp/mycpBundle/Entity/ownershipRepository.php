...
class ownershipRepository extends EntityRepository
{
    ...
    public function getSimilars($ownership, $rooms)
    {
        ...
        return $em->createQuery($queryString)
            ->setParameters(array
            ("province" => $ownership->getOwnAddressProvince()->getProvId(),
...
//;";
    ...
    public function getFacturacionMes($code)
    {
        $em = $this->getEntityManager();
        $query_string = "select t.anno,  t.mes,t.casa,t.messs, format(t.facturacion,2) AS facturacion from (
select year(gres.gen_res_date) as anno, MONTH(gres.gen_res_date) as mes, monthname(gres.gen_res_date)as messs,
o.own_mcp_code as casa, SUM(IF(p.current_cuc_change_rate IS NOT NULL, (p.payed_amount/p.current_cuc_change_rate) ,((owres.own_res_total_in_site * o.own_commission_percent)/100))) as facturacion 
from ownership o join generalreservation gres on gres.gen_res_own_id = o.own_id join ownershipreservation owres on owres.own_res_gen_res_id = gres.gen_res_id 
join booking b on b.booking_id = owres.own_res_reservation_booking join payment p on p.booking_id = b.booking_id group by YEAR(gres.gen_res_date), MONTH(gres.gen_res_date), gres.gen_res_own_id) t 
where    t.anno = year(now()) and  t.mes=MONTH(now())-1 and t.casa = '$code'";


        $stmt = $em->getConnection()->query($query_string);
//        $query = $em->createQuery($query_string);
//
//        if ($code != null)
//            $query->setParameter("date", $code);
    ...
    public function getRankingCasa($code)
    {
        $em = $this->getEntityManager();
        $query_string = "select format(own_ranking,2)as ranking from ownership where own_mcp_code = '$code'";
