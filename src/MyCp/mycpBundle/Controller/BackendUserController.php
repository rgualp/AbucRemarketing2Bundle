...
class BackendUserController extends Controller
{
    function new_user_staffAction($id_role,Request $request)
    {
        ...
        return $this->render('mycpBundle:user:newUserStaff.html.twig',array('form'=>$form->createView(),'data'=>$data,'id_role'=>$id_role));
    }
    ...
    function edit_user_staffAction($id_user,Request $request)
    {
        ...
        return $this->render('mycpBundle:user:newUserStaff.html.twig',array('form'=>$form->createView(),'data'=>$data,'id_role'=>'','edit_user'=>$id_user));
    }
    ...
    function new_user_touristAction($id_role,Request $request)
    {
        ...
        {
            $post = $request->request->getIterator()->getArrayCopy();
            $user_db=$em->getRepository('mycpBundle:user')->findBy(array('user_name'=>$post['mycp_mycpbundle_client_touristtype']['user_name']));
            if($user_db)
            {
                $data['error']='Ya existe un usuario con ese nombre.';
            }
            ...
            if($form->isValid())
            {
                if(!isset($data['error']))
                {
                    ...
                    $service_log= $this->get('log');
                    $service_log->saveLog('Create entity for user '.$post['mycp_mycpbundle_client_touristtype']['user_name'], BackendModuleName::MODULE_USER);
    ...
    function new_user_partnerAction($id_role,Request $request)
    {
        ...
        {
            $post = $request->request->getIterator()->getArrayCopy();
            $user_db=$em->getRepository('mycpBundle:user')->findBy(array('user_name'=>$post['mycp_mycpbundle_client_partnertype']['user_name']));
            if($user_db)
            {
                $data['error']='Ya existe un usuario con ese nombre.';
            }
            ...
            if($form->isValid())
            {
                if(!isset($data['error']))
                {
                   $dir=$this->container->getParameter('user.dir.photos');
                    ...
                    $this->get('session')->getFlashBag()->add('message_ok',$message);
                    $service_log= $this->get('log');
                    $service_log->saveLog('Create entity for user '.$post['mycp_mycpbundle_client_partnertype']['user_name'], BackendModuleName::MODULE_USER);
                    return $this->redirect($this->generateUrl('mycp_list_users'));
                }
            }
            else
    ...
    function list_userAction($items_per_page,Request $request)
    {
        ...
        if($request->getMethod()=='POST' && $filter_user_name=='null' && $filter_role=='null' && $filter_city=='null'
