...
class SendEmailUpdateApkCommand extends ContainerAwareCommand
{
	private $em;
	...
	{
		$this
			->setName('mycp_task:send_email_update_apk')
			->setDescription('Send notification to user ')
            ->addArgument("init", InputArgument::OPTIONAL,"Send to certain email")
            ->addArgument("final", InputArgument::OPTIONAL,"Send to certain email");
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$mail_fails= array();
		$mail_success= array();
        $init = $input->getArgument("init");
        $final = $input->getArgument("final");
		$mails= array();
		$ownerships= $this->getOwnerships($init,$final);
	...
	protected function getOwnerships($init,$final){
        ...
        $sql.="AND o.own_id >=". $init." AND o.own_id <=".$final."";
