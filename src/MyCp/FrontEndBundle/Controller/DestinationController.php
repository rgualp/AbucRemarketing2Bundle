...
//                           {% set url_image = asset("uploads/destinationImages/" ~ (des_photo)) %}
                ...
                if (count($dest_prov) > 0){
                    foreach ($dest_prov as $key => $item){
                        ...
                        foreach ($item as $li){
                            $html .= '<li style="display: inline-block; padding: 5px; text-align: center;"><a href="'.$this->generateUrl('newfrontend_details_destination', array('destination_name' => $li['des_name_for_url'])).'"><img src="'.$this->container->get('templating.helper.assets')->getUrl('uploads/destinationImages/' . $li['photo']).'" width="150" /> </br> <span class="text-center">'.$li['des_name'].'</span></a></li>';
                        }
                        $html .= '</ul>';
                        $prov[$key]['html'] = $html;
                    }
