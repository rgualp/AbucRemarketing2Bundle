...
class PaymentController extends Controller
{
    ...
    public function postFinancePaymentAction($bookingId, $method = "VISA")
    {
        ...
        $postFinanceData = $this->getPostFinanceViewData($booking, $user, $travelAgency, $method);
    ...
    public function skrillReturnAction($bookingId)
    {
        ...
        $pollingUrl = $this->generateUrl('partner_payment_poll_payment',
        ...
        $timeoutUrl = $this->generateUrl('partner_dashboard_cart',
            array(), UrlGeneratorInterface::ABSOLUTE_URL);

        $cancelUrl = $this->generateUrl('partner_dashboard_cart',
            array(), UrlGeneratorInterface::ABSOLUTE_URL);

        $failedUrl = $this->generateUrl('partner_dashboard_cart',
            array(), UrlGeneratorInterface::ABSOLUTE_URL);

        return $this->render(
            'PartnerBundle:Payment:waitingForPayment.html.twig',
            array(
    ...
    {
        $payUrl = $this->generateUrl('partner_payment_skrill', array('bookingId' => $bookingId), true);
        return $this->render(
            'PartnerBundle:Payment:skrillPaymentTest.html.twig', array('payUrl' => $payUrl));
    }
    ...
    {
        $urltopost = $this->generateUrl('partner_payment_skrill_status', array(), true);
        ...
        return $this->redirect($this->generateUrl(
            'partner_payment_skrill_return', array('bookingId' => $bookingId)));
    }
    ...
    private function getSkrillViewData(booking $booking, user $user, paTravelAgency $agency)
    {
        ...
        $skrillData = array(
            ...
            'transaction_id' => $bookingId,
            'return_url' => $this->generateUrl('partner_payment_skrill_return', array('bookingId' => $bookingId), true),
            'return_url_text' => $translator->trans('SKRILL_RETURN_TO_MYCP'),
            'cancel_url' => $this->generateUrl('partner_payment_skrill_cancel', array(), true),
            'status_url' => $this->generateUrl('partner_payment_skrill_status', array(), true),
            ...
            'country' => (isset($country)) ? $country->getCoCode() : "",
    ...
    private function getPostFinanceViewData(booking $booking, user $user, paTravelAgency $travelAgency, $method = "POSTFINANCE")
    {
        $bookingId = $booking->getBookingId();
        $translator = $this->get('translator');
        $locale = $this->getRequest()->getLocale();
